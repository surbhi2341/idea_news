...
const WatchPage = () => {
  ...
  const fetchShorts = async () => {
    setLoading(true);
    try {
      // Fetch backend videos (tries ShortVideo type first, falls back to all videos)
      const res = await axios.get('/api/media/videos');
      if (res.data.success && res.data.videos.length > 0) {
        const shorts = res.data.videos.filter((v) => v.type === 'ShortVideo');
        setVideos(shorts.length > 0 ? shorts : res.data.videos);
      }
    } catch (err) {
      console.error('Failed to fetch shorts from backend', err);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight">
              {language === 'Hindi' ? 'वीडियो स्टोरीज़ (वॉच)' : 'Video Stories (Watch)'}
            </h1>
          </div>
          <p className="text-xs text-slate-400 font-semibold">
            {language === 'Hindi'
              ? 'बैकएंड से अपलोड की गई सभी शॉर्ट्स खबरें'
              : 'Short news video clips fetched from backend'}
          </p>
        </div>
        ...
        {!loading && filteredVideos.length === 0 && (
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-12 text-center text-slate-400 text-sm">
            {language === 'Hindi'
              ? 'बैकएंड से कोई शॉर्ट्स वीडियो नहीं मिली।'
              : 'No short videos found from backend.'}
          </div>
        )}

        {/* ── 9:16 Shorts Grid Cards (Dainik Bhaskar UI) ── */}
        {!loading && filteredVideos.length > 0 && (
